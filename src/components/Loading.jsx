...
const Loading = ({ fullScreen = false, size = 8 }) => {
  const sizeClass = `w-${size} h-${size}`;
  return (
    <div
      className={`${
        fullScreen ? "fixed inset-0 z-50 bg-black bg-opacity-30 flex" : "flex"
      } justify-center items-center`}
    >
      <div role="status" aria-label="Loading">
        <svg
          aria-hidden="true"
          className={`${sizeClass} text-gray-200 animate-spin dark:text-gray-600 fill-primary`}
          viewBox="0 0 100 101"
          fill="none"
